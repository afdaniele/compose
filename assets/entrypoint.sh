...
if [ ! "$(getent group "${GID}")" ]; then
    echo "Creating a group 'compose' with GID:${GID} for the user ${COMPOSE_USER}"
    # create group
...
# add user to group
echo "Adding user ${COMPOSE_USER} to the group ${GNAME} (GID:${GID})."
sudo usermod -aG ${GNAME} ${COMPOSE_USER}

# give the newly created group r/w access to the user-data dir
sudo chmod -R g+r "${COMPOSE_USERDATA_DIR}"
sudo chmod -R g+w "${COMPOSE_USERDATA_DIR}"
...
if [ "${HTTP_PORT}" != "${DEFAULT_HTTP_PORT}" ]; then
    ...
    sudo sed -i "s/listen ${DEFAULT_HTTP_PORT} default_server/listen ${HTTP_PORT} default_server/g" /etc/nginx/sites-available/default
fi
...
if [ "${HTTPS_PORT}" != "${DEFAULT_HTTPS_PORT}" ]; then
    ...
    sudo sed -i "s/listen ${DEFAULT_HTTPS_PORT} default_server/listen ${HTTPS_PORT} default_server/g" /etc/nginx/sites-available/default
fi
...
compose_terminate() {
    ...
    sudo kill -INT $(pgrep -P $$) 2> /dev/null
}
