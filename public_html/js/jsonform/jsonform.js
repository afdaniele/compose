...
jsonform.fieldTemplate = function(inner) {
  return '<div ' +
    ...
    '<% }%>' +
    'class="input-group jsonform-group jsonform-error-<%= keydash %>' +
    ...
    '">' +
    '<% if (!elt.notitle) { %>' +
      '<span class="input-group-text w-25" id="basic-addon1"><%= node.title ? node.title : node.name %></span>' +
    '<% } %>' +
    ...
    '</div>' +
    '<% if (node.description) { %>' +
      '<div class="w-100 help-block text-right jsonform-description"><%= node.description %></div>' +
    '<% } %>';
};
...
jsonform.elementTypes = {
  ...
  'fieldset': {
    'template': '<fieldset class="input-group jsonform-error-<%= keydash %> <% if (elt.expandable) { %>expandable<% } %> <%= elt.htmlClass?elt.htmlClass:"" %>" ' +
      '<% if (id) { %> id="<%= id %>"<% } %>' +
      '>' +
      '<% if (node.title || node.legend) { %><legend role="treeitem" aria-expanded="false"><%= node.title || node.legend %></legend><% } %>' +
      '<% if (elt.expandable) { %><div class="input-group"><% } %>' +
      '<%= children %>' +
  ...
  'selectfieldset': {
    'template': '<fieldset class="tab-container <%= elt.htmlClass?elt.htmlClass:"" %>">' +
      ...
      '<% if (node.formElement.key) { %><input type="hidden" id="<%= node.id %>" name="<%= node.name %>" value="<%= escape(value) %>" /><% } else { %>' +
        '<a id="<%= node.id %>"></a><% } %>' +
      '<div class="tabbable">' +
        '<div class="input-group<%= node.formElement.hideMenu ? " hide" : "" %>">' +
