...
function get_package_latest_version($pkg) {
    ...
    foreach ($index['packages'][$pkg]['versions'] as $v => $vinfo) {
        ...
        if (is_null($compose_version) ||
            ...
            if (version_compare($ver($v), $ver($version), '>')) {
                $version = $v;
            }
        }
    }
...
if (count($in_out_conflicts) + count($up_out_conflicts) > 0) {
    foreach ($in_out_conflicts as $package) {
        $errors[] = sprintf('ERROR: The package "%s" is in the list of packages to install and uninstall', $package);
    }
    foreach ($up_out_conflicts as $package) {
        $errors[] = sprintf('ERROR: The package "%s" is in the list of packages to update and uninstall', $package);
    }
    Core::openAlert(AlertType::ERROR, implode('\n', $errors));
} else {
    if (isset($_GET['confirm']) && $_GET['confirm'] == '1') {
        // append version to packages
        $fcn = function ($x) use ($version_map) {
            return sprintf("%s==%s", $x, $version_map[$x]);
        };
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            foreach ($data as $package_id) {
                ...
                echo sprintf('&bullet; %s (<span class="mono" style="color:grey">%s</span>)',
                    $package_name,
                    $package_id
                );
            }
